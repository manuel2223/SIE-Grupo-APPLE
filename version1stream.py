...
class AppExamen(ctk.CTk):
    ...
    def conectar_nube():
        try:
            ...
            # 🟢 Streamlit convierte mágicamente el bloque del TOML en un diccionario de Python
            creds_dict = dict(st.secrets["gcp_service_account"])
        
            creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
            cliente = gspread.authorize(creds)
        
            # Guardamos la hoja en el estado de la sesión de Streamlit para no reconectar en cada clic
            st.session_state.cliente = cliente
            st.session_state.hoja = cliente.open("Banco de Preguntas Examenes SAEL (respuestas)").sheet1
            return True
        except Exception as e:
            st.error(f"No se pudo conectar a la nube: {e}")
            return False
