...
NOMBRE_EXCEL_NUBE = "Banco de Preguntas SAEL (Maestro) (respuestas)"
...
def generar_examen_y_pdf(lista_preguntas):
    ...
    for fila in lista_preguntas:
        for i in range(1, 21): 
            ...
            if col_enunciado not in fila or not fila[col_enunciado]:
                continue
            ...
            preguntas_validas.append({'enunciado': str(enunciado).replace('\n', ' '), 'opciones': opciones_mezclar})

    if not preguntas_validas: return False
