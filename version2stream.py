...
def generar_archivos_examen(lista_preguntas, tipo_examen, instrucciones):
    ...
    for numero, pregunta in enumerate(preguntas_normales, start=1):
        ...
        for indice, opcion in enumerate(pregunta['opciones']):
            letra = letras[indice]
            pdf.set_x(38) 
            pdf.multi_cell(0, 6, f"{letra}. {opcion['texto']}")
    ...
    pdf_plantilla = PDF_Examen(tipo_examen)
    pdf_plantilla.add_page()
    pdf_plantilla.set_y(55) # 🟢 EMPUJÓN PARA NO PISAR EL LOGO
...
# ==========================================
st.title("📝 Generador de Exámenes SAEL 3.0")
...
if 'df' in st.session_state:
    ...
    tab1, tab2 = st.tabs(["📄 Generador Nuclear", "🛠️ Gestor de Preguntas"])
    ...
    with tab2:
        ...
        with st.expander("🚨 Zona de Peligro: Eliminar Convocatoria Entera"):
